...
type QuestionnaireVersionRow =
  Database["public"]["Tables"]["questionnaire_versions"]["Row"];
...
function selectRelevantBatch(batches: RelevantBatchContext[], nowIso: string) {
  return (
    batches.find(
      (batch) =>
        batch.status === "open" ||
...
export const getEffectiveQuestionnaireContext = cache(
  async (supabase: SupabaseClient<Database>) => {
    ...
    if (relevantBatch) {
      const { data: version, error: versionError } = await supabase
        ...
        .select(
          "id, version_no, title, description, status, matching_policy_json",
        )
