...
export default async function AdminBatchesPage({
  ...
  const [batches, publishedQuestionnaires, resolvedSearchParams] = await Promise.all([
    listBatches(),
    listPublishedQuestionnaireOptions(),
    searchParams,
  ]);
    ...
    <div className="grid gap-6">
      <SurfaceCard>
        <SectionHeader
          eyebrow="批次运营"
          title="创建并推进每周批次"
          description="系统同一时间只允许存在一个当前轮次。批次编号、code 和展示轮次号都由系统自动递增生成，状态推进由管理员手动执行。"
        />
        {error ? (
        ...
        <form action={createBatchAction} className="mt-8 grid gap-5">
          <div className="grid gap-5 lg:grid-cols-2">
            <SelectField name="questionnaireVersionId" label="已发布问卷版本" required>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <Field name="signupEndAt" label="报名截止时间" type="datetime-local" required />
            <Field name="matchRunAt" label="匹配计算时间" type="datetime-local" required />
            <Field
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={publishedQuestionnaires.length === 0}>
              新建 draft 批次
            </Button>
          </div>
        </form>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="text-secondary-foreground/80 mt-3 text-sm leading-7">
                    状态 {batch.status}，问卷版本 {batch.questionnaireVersionId}，报名截止{" "}
                    {formatDateTime(batch.signupEndAt)}，结果发布时间{" "}
                    {formatDateTime(batch.resultPublishAt)}。
                  </p>
                  <p className="text-secondary-foreground/80 mt-3 text-sm leading-7">
                    报名 {batch.participationCount} 人，已锁定 {batch.lockedCount} 人，已发布结果{" "}
                    {batch.publishedResultCount} 条，未匹配 {batch.unmatchedCount} 人。
                  </p>
